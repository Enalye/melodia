...
final class TabsList : GuiElement {
    ...
    this() {
        ...
        size(Vec2f(getWindowWidth(), tabsHeight));
        setEventHook(true);
    ...
    override void onEvent(Event event) {
        ...
        case mouseWheel:
            ...
            canvas.position = canvas.position.clamp(canvas.size / 2f,
                    Vec2f(_box.size.x - canvas.size.x / 2f, canvas.size.y));
            break;
        default:
            break;
        }
...
final private class TabButton : GuiElement {
    ...
    this(TabsList tabs, TabData tabData) {
        ...
        GuiState hiddenState = {
            alpha: 0f, time: .5f, easing: getEasingFunction(Ease.sineInOut)
        };

        GuiState visibleState = {
            time: .5f, easing: getEasingFunction(Ease.sineInOut)
        };
        ...
        GuiState startState = {
            scale: Vec2f(0f, 1f), time: .5f, easing: getEasingFunction(Ease.sineInOut)
        };
